...
# Each category maps to a list of sets of circle_positions
categories = {
    "BX1112": [
        [(1, 1), (1, 2), (1, 3), (2, 4)],
    ],
...
def format_label(cat_name: str) -> str:
    ...
    - Insert space before 'slow' and 'fast'
    - Replace '124' with '124/134'
    """
    label = cat_name
    if "124" in label:
...
for cat_name, diagrams in categories.items():
    n = len(diagrams)
    ncols = min(n, 3)                # max 3 per row
    nrows = (n + 2) // 3             # ceil division

    # adjust figsize dynamically (4x4 per subplot)
    fig, axes = plt.subplots(nrows, ncols, figsize=(4*ncols, 4*nrows))

    # Flatten axes safely into list
    if isinstance(axes, plt.Axes):       # only 1 diagram
        axes = [axes]
    else:
        axes = axes.flatten()
